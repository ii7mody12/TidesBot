...
class Pokedex:
    ...
    @pokemon.command(name="pokeversion", pass_context=True)
    async def _version_pokemon(self, ctx):
        ...
        """
        await ctx.message.channel.send("You are running pokedex version {}".format(self.version))

    @pokemon.command(aliases=["dex"], pass_context=True)
    async def pokedex(self, ctx, *, pokemon: str):
        ...
        try:
            ...
            await ctx.message.channel.send('A Pokémon with that name could not be found.')
        else:
    ...
    @pokemon.command(name="moves", pass_context=True)
    ...
    async def item(self, ctx, *, item_name: str):
        ...
        try:
            ...
            await ctx.message.channel.send('An item with that name could not be found.')
        else:
    ...
    @pokemon.command(name="tmset", pass_context=True)
    async def _tmset_pokemon(self, ctx, generation: str, *, poke: str):
        ...
        if poke.title() in tm_exceptions:
            return await ctx.message.channel.send("This Pokémon cannot learn TMs.")

        gen = switcher.get(generation, 7)
        print(generation + ", " + poke)
        print(gen)
        try:
            tm_url = "http://pokemondb.net/pokedex/{}/moves/{}".format(poke, generation)
            print(str(tm_url))
            with aiohttp.ClientSession() as session:
                async with session.get(tm_url) as response:
                    ...
                    if len(moves) <= 30:
                        ...
                        await ctx.message.channel.send("```{}```".format(t))
                    else:
                        ...
                        await ctx.message.channel.send("```{}```".format(t1))
                        await ctx.message.channel.send("```{}```".format(t2))
        except IndexError:
            await ctx.message.channel.send("Oh no! That Pokémon was not available in that generation.")
        except AttributeError:
            await ctx.message.channel.send("Could not locate a Pokémon with that name.")

    @pokemon.command(name="location", pass_context=True)
